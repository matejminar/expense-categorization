...
import { CATEGORIES } from "./api/suggest-category/route"
...
export default function FinanceTracker() {
  ...
  const getCurrentLocation = () => {
    ...
    setTimeout(() => {
      const locations = [
        { lat: 48.2082, lng: 16.3719 }, // Vienna city center
        { lat: 48.2035, lng: 16.3618 }, // Naschmarkt area
        { lat: 48.2167, lng: 16.3833 }, // Prater park
        { lat: 48.2100, lng: 16.3700 }, // Stephansplatz
        { lat: 48.1865, lng: 16.3126 }, // Schönbrunn Palace
      ]
  ...
  useEffect(() => {
    ...
    if (savedExpenses) {
      const parsed = JSON.parse(savedExpenses).map((exp: any) => ({
        ...exp,
        datetime: typeof exp.datetime === 'string' ? exp.datetime : new Date(exp.timestamp).toISOString(),
      }))
      setExpenses(parsed)
    }
  }, [])
                    ...
                    >
                      <div className="text-left w-full">
                        ...
                        {suggestion.reasoning && (
                          <div
                            className="text-xs text-gray-500 mt-1 break-words whitespace-pre-line max-h-12 overflow-hidden text-ellipsis cursor-help"
